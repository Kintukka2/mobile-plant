...
The setup needs a distribution certificate, an App Store provisioning
profile, and an App Store Connect API key. Three runs were spent trying to
get away with two — see the section below for why that failed — so take the
profile as non-negotiable rather than an oversight.
...
`MSYS_NO_PATHCONV=1` is not optional: Git Bash otherwise rewrites the
leading `/` of the subject into a Windows path and the request comes out
with rubbish in it.
...
openssl pkcs12 -export -inkey dist.key -in dist.pem -out dist.p12 -passout pass:YOURPASSWORD
openssl base64 -A -in dist.p12 -out dist.p12.b64
```

`-passout` rather than the interactive prompt, because Git Bash does not
reliably render OpenSSL's password prompt — it looks like a hang.

Keep `dist.key` and `dist.p12`. Replacing them costs one of three
certificate slots.

**2. An App Store provisioning profile.** developer.apple.com → Profiles →
**+** → Distribution → **App Store Connect** → App ID
`com.sproutevergreen.app` → the Apple Distribution certificate above.

**Name it exactly `Sprout App Store`.** That string is
`PROVISIONING_PROFILE_SPECIFIER` in `project.pbxproj` and the
`provisioningProfiles` entry in the export options. The workflow reads the
name out of the profile and fails with a plain message if the two disagree,
rather than letting it surface later as an opaque signing error.

Download it, then:

```bash
openssl base64 -A -in Sprout_App_Store.mobileprovision -out profile.b64
```

It expires after a year. Renewing it is the same three clicks and a new
secret.

**3. An App Store Connect API key** — App Store Connect → Users and Access →
Integrations → App Store Connect API → **+**, role **App Manager**. The
`.p8` downloads exactly once. Note the Key ID beside it and the Issuer ID
above the table. Only the upload uses this now; signing does not.
...
| `IOS_PROVISIONING_PROFILE` | the contents of `profile.b64` |
...
### Why signing is manual, in three failed runs

Automatic signing cannot work on this account, and the reason only became
visible one error at a time. Capacitor's template sets
`CODE_SIGN_IDENTITY = "iPhone Developer"` at the project level in both
configurations while the target sets `CODE_SIGN_STYLE = Automatic`.

**Run 1**, template as generated:
...
**Run 2**, after changing Release to `"Apple Distribution"`:
...
So with automatic signing, *any* hand-written identity is a conflict
whatever its value. Both lines came out.

**Run 3**, automatic signing left to decide for itself:
...
That is the answer. **Automatic signing archives with a development
identity** and only re-signs for distribution at export time, so it wants a
development profile — and Apple will not issue one for a team with no
registered devices. There are none on this account, and a CI runner is never
going to be one. `-allowProvisioningUpdates` cannot help: it lets Xcode talk
to Apple, but Apple is the thing saying no.

The Release configuration is therefore manual:

```
CODE_SIGN_IDENTITY = "Apple Distribution";
CODE_SIGN_STYLE = Manual;
PROVISIONING_PROFILE_SPECIFIER = "Sprout App Store";
```

Debug stays automatic and untouched.

Those settings live in `project.pbxproj` rather than on the `xcodebuild`
command line on purpose. A build setting passed on the command line applies
to **every** target in the workspace, and a `PROVISIONING_PROFILE_SPECIFIER`
bound to `com.sproutevergreen.app` leaking onto the Pods frameworks is its
...
Three runs have reached `xcodebuild archive`. `npm ci`, CocoaPods,
`cap sync` and the keychain import all worked on a real runner, so those are
no longer guesses, and the image carries Xcode 26.6 with the iOS 26.5 SDK.
All three failed on signing, for the three reasons recorded above.

The profile install step, the archive under manual signing, the export, the
`.ipa`, `altool --validate-app` and the upload have still never run once.
